...
export const createImageCaptureWindow = () => {
  ...
  if (isProd) {
    win.loadURL('app://./image-capture');
  } else {
    const port = process.argv[2];
    win.loadURL(`http://localhost:${port}/image-capture`);
  }
  ...
  win.webContents.on('did-finish-load', () => {
    ...
    win.focus();
    win.setIgnoreMouseEvents(false);
    console.log('[Image Capture] 창 로드 완료, 포커스 설정됨');
  });
